...
import config, { URLS, baseURL, azureURL } from './config';
import 'whatwg-fetch';

const getAuthOptions = (method, parturl, data={}, queryParams={}) => {
...
export default {
  azureAuthenticate: (data) => {
    ...
    //
    // getAllOrganisation: () => {
    //   const authOptions = getAuthOptions(method, URLS.Organisation);
    //   return runAxios(authOptions);
    // }
    //
    // addUsers: (data) => {
    //   const authOptions = getAuthOptions(method, URLS.User, data);
    //   return runAxios(authOptions)
    // }
}
